export default class FFT {
  ...
  createComplexArray() {
    const res: number[] = new Array(this._csize);
  ...
  _singleTransform2(outOff: number, off: number, step: number) {
    const out = this._out;
  ...
  _singleTransform4(outOff: number, off: number, step: number) {
    const out = this._out;
    const data = this._data;
  ...
  //
  // NOTE: Only called for len=4
  _singleRealTransform2(outOff: number, off: number, step: number) {
    const out = this._out;
    const data = this._data;
  ...
  //
  // NOTE: Only called for len=8
  _singleRealTransform4(outOff: number, off: number, step: number) {
    const out = this._out;
    const data = this._data;
    const inv = this._inv ? -1 : 1;
    const step2 = step * 2;
